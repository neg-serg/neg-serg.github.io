...
компании(да, такое внезапно бывает).
...
не представляет. Это можно использовать когда вим тормозит непонятно почему.
...
что есть в продуктах jetbrains. Чтобы он(fzf) работал ещё понадобится tmux,
...
расширений. В частности я очень люблю использовать *Agit*
